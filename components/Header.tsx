...
export default function Header() {
  ...
  const baseButtonClasses = "p-2 rounded-full hover:bg-white/10 transition";

  return (
    <>
      <header className="w-[340px] h-16 z-50 fixed flex items-center p-3 border border-white/25 shadow-xs shadow-white rounded-full bg-black/60 backdrop-blur-[20px] top-[10px] left-1/2 -translate-x-1/2">
        <div className="flex w-full justify-between items-center">
          ...
          <Link href={"/welcome"} className={`${baseButtonClasses} flex gap-1 items-center`}>
          ...
          <nav className="flex flex-col gap-3 mt-2">
            {menuItems.map((item) => (
              <Link
                ...
                className={`${baseButtonClasses} flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-white/10 transition`}
              >
